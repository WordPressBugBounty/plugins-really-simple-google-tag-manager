 ...
 * Class.
 */
if ( ! class_exists( 'Simple_Google_Tag_Diagnostic_Data' ) ) {
    class Simple_Google_Tag_Diagnostic_Data {
        ...
        private function __construct() {
            $this->project_name = 'Really Simple Google Tag Manager';
            $this->project_type = 'wordpress-plugin';
            $this->project_version = SIMPLE_GOOGLE_TAG_VERSION;
            ...
            $this->project_pro_slug = 'really-simple-google-tag-manager-pro/really-simple-google-tag-manager-pro.php';
            ...
            if ( get_option( 'simple_googletag_diagnostic_data_agreed' ) === 'yes' || get_option( 'simple_googletag_diagnostic_data_notice' ) === 'no' ) {
                return;
            }
            ...
            add_action( 'wp_ajax_simple_googletag_diagnostic_data', function () {
                $this->process_data();
            } );
        }
        ...
        private function is_show_core_notice() {
            $result = get_option( 'simple_googletag_diagnostic_data_notice', 'yes' );
        ...
        private function process_data() {
            ...
            if ( ! wp_verify_nonce( $nonce, 'simple_googletag_diagnostic_data_nonce' ) ) {
                $errormessage = array(
                    'message'  => __('Nonce Varification fail','simple-googletag')
                );
            ...
            update_option( 'simple_googletag_diagnostic_data_agreed', $agreed );
            update_option( 'simple_googletag_diagnostic_data_notice', $notice );
        ...
        public function should_show_notice() {
            ...
            foreach ( $sibling_plugins as $plugin_slug => $options ) {
                if ( get_option( $options['agreed'] ) === 'yes' ) {
                    update_option( 'simple_googletag_diagnostic_data_agreed', 'yes' );
                    update_option( 'simple_googletag_diagnostic_data_notice', 'no' );
                    return false;
                }
            }
            ...
            if ( isset( $ht_diagnostic_notice_owner ) && $ht_diagnostic_notice_owner !== 'simple_googletag' ) {
                return false;
            }
            $ht_diagnostic_notice_owner = 'simple_googletag';
        ...
        private function show_core_notice() {
            ...
            $message_l2 = sprintf( esc_html__( 'Server information (Web server, PHP version, MySQL version), WordPress information, site name, site URL, number of plugins, number of users, your name, and email address. You can rest assured that no sensitive data will be collected or tracked. %1$sPrivacy Policy%2$s', 'simple-googletag' ), '<a target="_blank" href="' . esc_url( $this->privacy_policy ) . '">', '</a>' );

            $button_text_1 = esc_html__( 'Count Me In', 'simple-googletag' );
            $button_link_1 = add_query_arg( array( 'simple_googletag-diagnostic-data-agreed' => 1 ) );

            $button_text_2 = esc_html__( 'No thanks', 'simple-googletag' );
            $button_link_2 = add_query_arg( array( 'simple_googletag-diagnostic-data-agreed' => 0 ) );
            ?>
            <div class="simple_googletag-diagnostic-data-style"><style>.simple_googletag-diagnostic-data-notice,.woocommerce-embed-page .simple_googletag-diagnostic-data-notice{padding-top:.75em;padding-bottom:.75em;}.simple_googletag-diagnostic-data-notice .simple_googletag-diagnostic-data-buttons,.simple_googletag-diagnostic-data-notice .simple_googletag-diagnostic-data-list,.simple_googletag-diagnostic-data-notice .simple_googletag-diagnostic-data-message{padding:.25em 2px;margin:0;}.simple_googletag-diagnostic-data-notice .simple_googletag-diagnostic-data-list{display:none;color:#646970;}.simple_googletag-diagnostic-data-notice .simple_googletag-diagnostic-data-buttons{padding-top:.75em;}.simple_googletag-diagnostic-data-notice .simple_googletag-diagnostic-data-buttons .button{margin-right:5px;box-shadow:none;}.simple_googletag-diagnostic-data-loading{position:relative;}.simple_googletag-diagnostic-data-loading::before{position:absolute;content:"";width:100%;height:100%;top:0;left:0;background-color:rgba(255,255,255,.5);z-index:999;}.simple_googletag-diagnostic-data-disagree{border-width:0px !important;background-color: transparent!important; padding: 0!important;}.simple_googletag-diagnostic-data-list-toogle{cursor:pointer;color:#2271b1;text-decoration:none;}</style></div>
            <div class="simple_googletag-diagnostic-data-notice notice notice-info">
                <p class="simple_googletag-diagnostic-data-message"><?php echo wp_kses_post( sprintf( esc_html__( 'Want to help make %2$s%1$s%3$s even more awesome? Allow %1$s to collect diagnostic data and usage information. (%4$swhat we collect%5$s)', 'simple-googletag' ), esc_html( $this->project_name ), '<strong>', '</strong>', '<a href="#" class="simple_googletag-diagnostic-data-list-toogle">', '</a>' ) ); ?></p>
                <p class="simple_googletag-diagnostic-data-list"><?php echo wp_kses_post( $message_l2 ); ?></p>
                <p class="simple_googletag-diagnostic-data-buttons">
                    <a href="<?php echo esc_url( $button_link_1 ); ?>" class="simple_googletag-diagnostic-data-button simple_googletag-diagnostic-data-agree button button-primary"><?php echo esc_html( $button_text_1 ); ?></a>
                    <a href="<?php echo esc_url( $button_link_2 ); ?>" class="simple_googletag-diagnostic-data-button simple_googletag-diagnostic-data-disagree button button-secondary"><?php echo esc_html( $button_text_2 ); ?></a>
                </p>
            </div>
            <div class="simple_googletag-diagnostic-data-script"><script type="text/javascript">;(function($){"use strict";function simple_googletagDissmissThanksNotice(noticeWrap){$('.simple_googletag-diagnostic-data-thanks .notice-dismiss').on('click',function(e){e.preventDefault();let thisButton=$(this),noticeWrap=thisButton.closest('.simple_googletag-diagnostic-data-thanks');noticeWrap.fadeTo(100,0,function(){noticeWrap.slideUp(100,function(){noticeWrap.remove()})})})};$(".simple_googletag-diagnostic-data-list-toogle").on("click",function(e){e.preventDefault();$(this).parents(".simple_googletag-diagnostic-data-notice").find(".simple_googletag-diagnostic-data-list").slideToggle("fast")});$(".simple_googletag-diagnostic-data-button").on("click",function(e){e.preventDefault();let thisButton=$(this),noticeWrap=thisButton.closest(".simple_googletag-diagnostic-data-notice"),agreed=thisButton.hasClass("simple_googletag-diagnostic-data-agree")?"yes":"no",styleWrap=$(".simple_googletag-diagnostic-data-style"),scriptWrap=$(".simple_googletag-diagnostic-data-script");$.ajax({type:"POST",url:ajaxurl,data:{action:"simple_googletag_diagnostic_data",agreed:agreed,nonce:'<?php echo wp_create_nonce( 'simple_googletag_diagnostic_data_nonce' );?>'},beforeSend:function(){noticeWrap.addClass("simple_googletag-diagnostic-data-loading")},success:function(response){response="object"===typeof response?response:{};let success=response.hasOwnProperty("success")?response.success:"no",notice=response.hasOwnProperty("notice")?response.notice:"no",thanks_notice=response.hasOwnProperty("thanks_notice")?response.thanks_notice:"";if("yes"===success){noticeWrap.replaceWith(thanks_notice);styleWrap.remove();scriptWrap.remove()}else if("no"===notice){noticeWrap.remove();styleWrap.remove();scriptWrap.remove()};noticeWrap.removeClass("simple_googletag-diagnostic-data-loading");simple_googletagDissmissThanksNotice()},error:function(){noticeWrap.removeClass("simple_googletag-diagnostic-data-loading")},})})})(jQuery);</script></div>
            <?php
        }
        ...
        private function get_thanks_notice() {
            $message = sprintf( esc_html__( 'Thank you very much for supporting %2$s%1$s%3$s.', 'simple-googletag' ), $this->project_name, '<strong>', '</strong>' );
            $notice = sprintf( '<div class="simple_googletag-diagnostic-data-thanks notice notice-success is-dismissible"><p>%1$s</p><button type="button" class="notice-dismiss"><span class="screen-reader-text"></span></button></div>', wp_kses_post( $message ) );
    ...
    Simple_Google_Tag_Diagnostic_Data::get_instance();
}